defmodule MetricsSpikeWeb.UpsertUserWebSessions do
  ...
  def call(params) do
    ...
    case status do
      :ok -> update_web_session(params, user_web_session)
      :error -> create_web_session(params)
    end
  end

  def create_web_session(item = %{"UserId" => user_id, "CreatedAt" => created_at}) do
    updated_at_value = Map.get(item, "CreatedAt")
    new_item = Map.put(item, "UpdatedAt", updated_at_value)

    MetricsDynamoDB.PutItem.call("PocUserWebSessions", new_item)
  end

  def update_web_session(params = %{"UserId" => _, "CreatedAt" => _}, user_web_session = %{UserId: _, CreatedAt: _, UpdatedAt: _}) do
    ...
    case DateTime.diff(event_date, web_session_created_at_date, :second)/3600 > @session_day_time_limit do
      true -> create_web_session(params)
      false ->
